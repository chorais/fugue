# **golden-test.md (v2.4)**  
**Golden Test Template — Tranche <X.Y>**  
...
```yaml
summary: >
  <One-sentence description of the ideal behaviour being validated.>
```

---

# 2. **Purpose**  
*(Why this golden test exists — grounded in governance envelopes)*

```yaml
purpose: >
  <Describe the deterministic, governance-aligned behaviour this test validates.>
```

---

# 3. **Success Mode Under Test**  
*(Explicitly defined success surface)*

```yaml
success-mode:
  type: "<determinism|identity|namespace|metadata|evaluator|governance|decor|lifecycle>"
  description: >
    <Describe the expected correct behaviour.>
...
# 4. **Inputs**  
*(Governed inputs required to validate correctness)*
...
*(Deterministic success expectations)*

```yaml
expected-success:
  determinism:
    - "<expected-deterministic-ordering>"
    - "<expected-deterministic-state-transition>"
  evaluator:
    - "<expected-evaluator-behaviour>"
  identity:
    - "<expected-identity-propagation>"
  namespace:
    - "<expected-namespace-placement>"
  metadata:
    - "<expected-metadata-propagation>"
  governance:
    - "<expected-governance-alignment>"
  replay:
    - "<expected-replay-determinism>"
```
